...
const ClientesTab: React.FC<ClientesTabProps> = ({
  A,
  globalSearch,
  clientesList,
  refreshClientes
    ...
    if (sTermLocal || sTermGlobal) {
      res = res.filter((c) => {
        ...
        const matchGlobal = !sTermGlobal || 
          ...
          phoneClean.includes(sTermGlobal.replace(/\D/g, '')) ||
        ...
        const matchLocal = !sTermLocal || 
          ...
          phoneClean.includes(sTermLocal.replace(/\D/g, '')) ||
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-9 gap-4 select-none">
        <motion.div
          ...
          className={`border cursor-pointer ${
            selectedSizeFilter === null
              ? 'border-brand-purple ring-2 ring-brand-purple/20 bg-purple-50/20 dark:bg-[#7C3AED]/5'
              : A.card
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <div className={`w-7 h-7 rounded-full flex items-center justify-center ${A.bgHover} transition-all`}>
              <ArrowUpRight size={14} className={A.textMuted} />
            </div>
          </div>
          <div className="mt-4 text-left">
            <h4 className={`text-3xl font-bold tracking-tight ${A.textPrimary}`}>
              {sizeStats.total}
            </h4>
        ...
        {[
          { size: 'PP', label: 'Tamanho PP', colorClass: 'text-cyan-500', barBg: 'bg-cyan-500', activeStyle: 'border-cyan-500 ring-2 ring-cyan-500/20 bg-cyan-50/20 dark:bg-cyan-500/5' },
          { size: 'P', label: 'Tamanho P', colorClass: 'text-emerald-500', barBg: 'bg-emerald-500', activeStyle: 'border-emerald-500 ring-2 ring-emerald-500/20 bg-emerald-50/20 dark:bg-emerald-500/5' },
          { size: 'M', label: 'Tamanho M', colorClass: 'text-[#A3E635]', barBg: 'bg-[#A3E635]', activeStyle: 'border-[#A3E635] ring-2 ring-[#A3E635]/20 bg-[#A3E635]/5' },
          { size: 'G', label: 'Tamanho G', colorClass: 'text-brand-purple', barBg: 'bg-brand-purple', activeStyle: 'border-brand-purple ring-2 ring-brand-purple/20 bg-purple-50/20 dark:bg-[#7C3AED]/5' },
          { size: 'GG', label: 'Tamanho GG', colorClass: 'text-blue-500', barBg: 'bg-blue-500', activeStyle: 'border-blue-500 ring-2 ring-blue-500/20 bg-blue-50/20 dark:bg-blue-500/5' },
          { size: 'XG', label: 'Tamanho XG', colorClass: 'text-rose-500', barBg: 'bg-rose-500', activeStyle: 'border-rose-500 ring-2 ring-rose-500/20 bg-rose-50/20 dark:bg-rose-500/5' },
          { size: 'XGG', label: 'Tamanho XGG', colorClass: 'text-amber-500', barBg: 'bg-amber-500', activeStyle: 'border-amber-500 ring-2 ring-amber-500/20 bg-amber-50/20 dark:bg-amber-500/5' },
          { size: 'Não Informado', label: 'Sem Tamanho', colorClass: 'text-slate-400', barBg: 'bg-slate-400', activeStyle: 'border-slate-400 ring-2 ring-slate-400/20 bg-slate-50/20 dark:bg-slate-400/5' }
        ].map((sObj) => {
          ...
          return (
            <motion.div
              ...
              className={`border cursor-pointer ${
                isSelected ? sObj.activeStyle : `${A.card} hover:border-slate-300 dark:hover:border-slate-600`
              } rounded-[24px] p-5 shadow-sm flex flex-col justify-between min-h-[140px] transition-all relative overflow-hidden`}
      ...
      <div className={`border ${A.card} rounded-[24px] overflow-hidden shadow-sm text-left`}>
        ...
        {/* Paginação footer */}
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 border-t border-slate-200 bg-[#f1f5f9]">
            <div className="text-xs text-slate-500 font-semibold">
              Exibindo <span className="font-bold text-slate-900">{Math.min((currentPage - 1) * itemsPerPage + 1, filteredClientes.length)}</span> a{' '}
              <span className="font-bold text-slate-900">{Math.min(currentPage * itemsPerPage, filteredClientes.length)}</span> de{' '}
              <span className="font-bold text-slate-900">{filteredClientes.length}</span> clientes
            </div>
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))}
                disabled={currentPage === 1}
                className={`p-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-all cursor-pointer ${
                  currentPage === 1 ? 'opacity-40 cursor-not-allowed text-slate-400' : 'text-brand-purple hover:border-brand-purple active:scale-95'
                }`}
              ...
              <button
                ...
                className={`p-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-all cursor-pointer ${
                  currentPage === totalPages ? 'opacity-40 cursor-not-allowed text-slate-400' : 'text-brand-purple hover:border-brand-purple active:scale-95'
                }`}
